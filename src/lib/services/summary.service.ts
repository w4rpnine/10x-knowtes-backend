...
export class SummaryService {
   ...
   */
  async acceptSummary(_userId: string, topicId: string, summaryId: string): Promise<{ summary_stat_id: string }> {
    // Using DEFAULT_USER_ID for development/testing
    const userId = DEFAULT_USER_ID;
    ...
    // 2. Get summary content - for now, we'll mock it as the database doesn't have these columns yet
    const summaryTitle = "Summary of Notes";
    const summaryContent = "This is a summary of your notes for this topic.";

    // Verify summary stat exists and belongs to user
    const { data: summaryStat, error: summaryStatError } = await this.supabase
      .from("summary_stats")
      .select("id, topic_id")
      .eq("id", summaryId)
    ...
    // Start transaction - in Supabase JS we use multiple operations and handle errors
    try {
